...
Notes:		Here we are using the sendto and recvfrom
		...
		In order to execute correctly this source code you may have to run as sudo.
***********************************************************************/
...
/* Done dealing with switches */
  ...
  server.sin_addr.s_addr = inet_addr ("192.168.3.24");	/*use any address */
...
tget (char *pFilename, struct sockaddr_in client, char *pMode, int tid)
{
  ...
  if (!strncasecmp (mode, "octet", 5) && !strncasecmp (mode, "netascii", 8))	/* these two are the only modes we accept */
	...
	if(filename[0]=='/') { //Il nuovo file name sarà uguale a quello vecchio eccetto il carattere backslash iniziale. 
	strncpy(filename, filename +1,127);
	printf("new name file: %s\n",filename);
		}
